...
def launch_meow_dashboard():
    """Launch the Meow Dashboard in the current app"""
    st.markdown(win95_header("Meow Dashboard"), unsafe_allow_html=True)
    
    # Info about the dashboard
    st.markdown("""
    <div style="margin-bottom: 20px;">
        <p>
            Meow Dashboard is a powerful real-time monitoring tool for financial markets. 
            It features auto-refreshing stock prices, technical indicators, news feed, and portfolio tracking.
        </p>
    </div>
    """, unsafe_allow_html=True)
    
    # Check if meow_dashboard.py exists
    if not os.path.exists("meow_dashboard.py"):
        st.error("meow_dashboard.py file not found. Please make sure it exists in the same directory as app.py.")
        return
    
    try:
        # Import and run the dashboard function directly from the module
        import meow_dashboard
        importlib.reload(meow_dashboard)
        meow_dashboard.main()
    except Exception as e:
        st.error(f"Error launching Meow Dashboard: {str(e)}")
        
        # Fallback method: Launch in a subprocess
        st.info("Attempting to launch dashboard as a separate process...")
        
        # Create launch button
        if st.button("Launch Meow Dashboard in New Window", use_container_width=True):
            try:
                # Start the process in the background
                subprocess.Popen([sys.executable, "meow_dashboard.py"])
                st.success("Meow Dashboard launched in a new window.")
            except Exception as e2:
                st.error(f"Failed to launch Meow Dashboard: {str(e2)}")
                st.info("Please run 'streamlit run meow_dashboard.py' in your terminal to launch the dashboard.")

def handle_content_section(selected_section, ticker):
    """Handle displaying the appropriate content based on the selection"""
    ...
    elif selected_section == "Meow Dashboard":
        launch_meow_dashboard()
...
def main():
    ...
    # All navigation options in a single dropdown - Added Meow Dashboard to the list
    selected_section = st.sidebar.selectbox(
        "Go to",
        ["Stock Dashboard", "Meow Dashboard", "News Dashboard", "Day Trader", "Backtesting", "Stock Analysis", "Technical Indicators", "Fundamental Analysis"]
    )
    ...
    # Sidebar for inputs (common across sections, except Meow Dashboard)
    if selected_section != "Meow Dashboard":
        ...
        # For Meow Dashboard, use a placeholder ticker (not actually used)
        ticker = ""
        
        # Terminal breadcrumb path at top for Meow Dashboard
        current_path = f"C:\\> STOCKS\\MEOW_DASHBOARD"
        st.markdown(f"<div style='color: #e6f3ff; font-family: monospace; margin-bottom: 10px;'>{current_path}</div>", unsafe_allow_html=True)
    
    # Display the appropriate content based on the selection
    handle_content_section(selected_section, ticker)
